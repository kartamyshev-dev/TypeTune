...
class Window(Gtk.ApplicationWindow):
    ...
    def dispatch(self, command, explicit=False):
        if self.closed: return
        if self.busy:
            if command != 'status' and not self.controls_busy:
                self.pending = (command, explicit)
        ...
        def work():
            try:
                state, error = self.requester(command)
    ...
    def complete(self, state, error):
        ...
        self.busy = False
        if self.pending:
            command, explicit = self.pending
            self.pending = None
            self.dispatch(command, explicit)
        else:
            self.controls_busy = False
            self.render()
        return False

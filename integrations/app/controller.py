...
def suggestions(command):
    ...
    return dict(error=False,message='Добавлено в «Слова и исключения».' if value['action']=='accept' else 'Предложение отклонено.')
...
def threshold_set():
    ...
    except ValueError: raise ValueError('Порог предложений: целое число от 1 до 10')
    correction_feedback.validate_threshold(value)
    def apply(document):
        threshold=document['learn_threshold']
        if active_runtime_control() is None: return
        try:
            confirmed=call(*SUGGESTION_BACKENDS['compatibility'],'SetLearnThreshold',GLib.Variant('(i)',(threshold,)))[0]
        except GLib.Error as error:
            raise RuntimeError('Порог сохранён, но не применён. Перезапустите режим коррекции') from error
        if confirmed != threshold:
            raise RuntimeError('Runtime не подтвердил порог')
    app_settings.update({'learn_threshold':value},apply=apply)
    return dict(error=False,message='Порог предложений сохранён.',learn_threshold=value)
...
def main():
    ...
    try:
        ...
        elif args.command=='threshold-set':
            print(json.dumps(threshold_set(),ensure_ascii=False))

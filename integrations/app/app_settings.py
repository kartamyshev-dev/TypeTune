...
DEFAULT = dict(version=1, mode='compatibility', automatic=True, autostart=False, learn_threshold=3)
REQUIRED = {'version', 'mode', 'automatic', 'autostart'}
...
def load(path=PATH):
    ...
    if (not isinstance(value,dict) or not REQUIRED <= set(value) or set(value)-set(DEFAULT)
            or type(value['version']) is not int or value['version']!=1
            or value['mode'] not in ('compatibility','ibus')
            or any(type(value[k]) is not bool for k in ('automatic','autostart'))):
    ...
    value.setdefault('learn_threshold', DEFAULT['learn_threshold'])
    if type(value['learn_threshold']) is not int or not 1 <= value['learn_threshold'] <= 10:
        raise ValueError('Неверный формат настроек TypeTune')
    return value
...
def update(changes, path=PATH, apply=None):
    if set(changes)-{'mode','automatic','learn_threshold'}: raise ValueError('Unsupported settings change')

...
class SuggestionEditor(Gtk.Window):
    def __init__(self,parent,requester=exchange):
        ...
        self.set_child(box)
        box.append(Gtk.Label(label='После настроенного числа подтверждений (по умолчанию три) автозамены предлагается исключение. После стольких же отдельных ручных исправлений — слово для автокоррекции. Пробел можно нажать до или после Double Shift. Ничего не добавляется без вашего подтверждения.',wrap=True,xalign=0))
        ...
        box.append(Gtk.Label(label='Добавленное слово можно удалить в «Слова и исключения». Отклонённое предложение сохраняется и не повторяется.',wrap=True,xalign=0))
    ...
    def complete(self,data,message):
        ...
        if data is not None:
            ...
            for proposal in data['proposals']:
                ...
                row.append(Gtk.Label(label=f'{verb} «{proposal["source"]}» → «{proposal["word"]}» · подтверждений: {count}?',wrap=True,xalign=0))

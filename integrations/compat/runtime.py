...
class Runtime:
    def __init__(self,stand=False):
        ...
        try: settings=app_settings.load()
        except (ValueError,OSError): settings=dict(app_settings.DEFAULT)
        self.feedback=correction_feedback.Feedback(
            path=correction_feedback.FEEDBACK_PATH,
            threshold=settings.get('learn_threshold',correction_feedback.DEFAULT_THRESHOLD),
...
<method name="Quit"/>'''
    ...
    def close(self):
        if self.closed:return False
        self.closed=True
        if hasattr(self,'feedback'):self.feedback.flush()
        if self.helper:
            try:self.helper.stdin.close()
            except OSError:pass

...
fn evaluate_auto_corpus() -> (CorpusStats, Vec<String>) {
    let clock = FakeClock::at_zero();
        ...
        .filter(|r| !r.starts_with('#') && !r.is_empty())
    {
        ...
        assert_eq!(cols.len(), 4, "corpus row must have 4 columns: {row}");
        let (split, kind, input, expected_word) = (cols[0], cols[1], cols[2], cols[3]);
        assert!(matches!(split, "dev" | "holdout"), "bad split {split}");
        assert!(matches!(kind, "positive" | "negative"), "bad kind {kind}");
        ...
        if kind == "positive" {
            stats.all.positive += 1;
            bucket.positive += 1;
            if inferred != expected {
                stats.all.missed += 1;
                bucket.missed += 1;
                failures.push(format!("MISS {split} {input} {expected_word}"));
            }
        } else {
            stats.all.negative += 1;
            bucket.negative += 1;
            assert_eq!(expected_word, input, "negative must be identity: {row}");
            if inferred != expected {
                stats.all.false_changes += 1;
                bucket.false_changes += 1;
                failures.push(format!("FALSE {split} {input} -> {inferred}"));
            }
        }
    }
    (stats, failures)
...
fn parse_baseline(text: &str) -> std::collections::BTreeMap<String, usize> {
    text.lines()
        .filter(|l| !l.starts_with('#') && !l.is_empty())
        .map(|l| {
            let (k, v) = l.split_once('=').expect("baseline key=value");
            (
                k.trim().to_string(),
                v.trim().parse().expect("baseline usize"),
            )
        })
        .collect()
}

fn baseline_for(stats: &CorpusStats) -> std::collections::BTreeMap<String, usize> {
    ...
    for (prefix, s) in [
        ...
        m.insert(format!("{prefix}_positive"), s.positive);
        m.insert(format!("{prefix}_missed"), s.missed);
        m.insert(format!("{prefix}_negative"), s.negative);
        m.insert(format!("{prefix}_false"), s.false_changes);
    }
    m
}
...
fn autocorrection_corpus_editor_and_inferred_agree() {
    let (stats, failures) = evaluate_auto_corpus();
    for line in &failures {
        eprintln!("{line}");
    }
    eprintln!(
        "AUTO-CORPUS all={} missed={} negative={} false={} | dev={}/{} miss, holdout={}/{} miss",
        stats.all.positive,
        stats.all.missed,
        stats.all.negative,
        stats.all.false_changes,
        stats.dev.missed,
        stats.dev.positive,
        stats.holdout.missed,
        stats.holdout.positive,
    );
    assert_eq!(
        stats.all.false_changes, 0,
        "false corrections are never allowed in the corpus"
    );

    // Regression gate (plan 57 D1): metrics must match the recorded baseline.
    // Improve coverage by updating data/corpus-baseline.txt in the same change
    // that explains the delta (new data, threshold policy, etc.).
    let baseline = parse_baseline(include_str!("../data/corpus-baseline.txt"));
    let actual = baseline_for(&stats);
    let mut diffs = Vec::new();
    for (key, value) in &actual {
        match baseline.get(key) {
            Some(want) if want == value => {}
            Some(want) => diffs.push(format!("{key}: baseline={want} actual={value}")),
            None => diffs.push(format!("{key}: missing in baseline, actual={value}")),
        }
    }
    for key in baseline.keys() {
        if !actual.contains_key(key) {
            diffs.push(format!("{key}: present in baseline, missing in actual"));
        }
    }
    assert!(
        diffs.is_empty(),
        "corpus metrics drifted from baseline:\n{}",
        diffs.join("\n")
    );
}
...
fn two_letter_words_preserve_context_and_reject_ambiguous_input() {
    ...
    for (input, expected) in [
        ...
        ("yt ", "не "),
        ("jy ", "он "),
        // Leeds source protection: rare RU tokens stay unchanged (D2).
        ("ещ ", "ещ "),
        ("шт ", "шт "),
        ("ye ", "ye "),
        ("lf ", "lf "),
...
fn user_words_and_exclusions_apply_to_both_profiles_without_changing_manual() {
    ...
    // Default merged lexicons (plan 57 D2) already admit клавиатуры.
    assert_eq!(
        inferred::suggest("rkfdbfnehs ", true)
            .map(|s| s.replacement)
            .unwrap_or_default(),
        "клавиатуры "
    );
    ...
    assert!(UserDictionary::new(vec!["hello".into(); 5001], vec![]).is_err());
}

...
/// Counts and provenance only — never prints word content (docs/16, docs/53).
fn show_dictionaries() {
    let lex = typetune_engine::lexicon_entry_counts();
    println!("Dictionaries:");
    println!(
        "  embedded RU {} entries / EN {} (primary+leeds+tech+plurals, plan 57 D2)",
        lex.ru_merged, lex.en_merged
    );
    println!(
        "  pilot RU {} / EN {} (MIT fixtures)",
        lex.ru_pilot, lex.en_pilot
    );
    println!(
        "  provenance: FrequencyWords CC BY-SA 4.0, Leeds CC BY 2.5, tech MIT — data/frequency/README.md"
    );
    let baseline = include_str!("../../typetune-engine/data/corpus-baseline.txt");
    let field = |key: &str| {
        baseline
            .lines()
            .find_map(|line| {
    ...
    println!(
        "  corpus baseline: all missed={} false={} (dev {}/{}, holdout {}/{})",
        field("all_missed"),
        field("all_false"),
        field("dev_missed"),
        field("dev_positive"),
        field("holdout_missed"),
        field("holdout_positive")
    );
    ...
    let words_path = config.join("typetune/words.json");
    let feedback_path = config.join("typetune/feedback.json");
    match std::fs::read(&words_path) {
        Ok(raw) => match serde_json::from_slice::<serde_json::Value>(&raw) {
            Ok(value) => println!(
                "  user words.json: {} words, {} exclusions (content not shown)",
                value
                    .get("words")
            ...
            Err(_) => println!("  user words.json: unreadable"),
        },
        Err(_) => println!("  user words.json: absent"),
    }
    match std::fs::read(&feedback_path) {
        Ok(raw) => match serde_json::from_slice::<serde_json::Value>(&raw) {
            Ok(value) => println!(
                "  feedback.json: {} pairs (content not shown)",
                value
                    .get("pairs")
                    .and_then(|v| v.as_array())
                    .map_or(0, |v| v.len())
            ),
            Err(_) => println!("  feedback.json: unreadable"),
        },
        Err(_) => println!("  feedback.json: absent"),
    }
}

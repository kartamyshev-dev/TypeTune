...
use tunetype_chatter::AntiChatter;
use tunetype_chatter::ChatterStats;
use tunetype_config::Config;
use tunetype_core::pipeline::Pipeline;
use tunetype_corrector::dictionary::Dictionary;
use tunetype_corrector::LayoutCorrector;
use tunetype_inject::VirtualKeyboard;
use tunetype_input::device_discovery::discover_keyboards;
use tunetype_input::EvdevSource;
use tunetype_snippets::SnippetExpander;
...
fn main() {
    ...
    let config_path = cli
        .config
        .unwrap_or_else(tunetype_config::ensure_config_exists);

    let config = match tunetype_config::load(&config_path) {
        Ok(c) => c,
        Err(e) => {
    ...
    match cli.command {
        ...
        Commands::Version => println!("tunetype {}", env!("CARGO_PKG_VERSION")),
    }
}
...
fn daemon_mode(config: Config) {
    tracing::info!("Starting TuneType daemon...");
    ...
    tunetype_tray::run_tray(tray_config, tray_enabled);
    ...
    remove_pid_file(&pid_path);
    tracing::info!("TuneType daemon stopped");
}
...
fn start_daemon() {
    ...
    let child = std::process::Command::new("tunetype")
        .arg("daemon")
...
fn show_status(config: &Config) {
    ...
    match read_pid(pid_path) {
        Some(pid) if process_alive(pid) => {
            println!("TuneType is running (PID: {})", pid);
        }
        Some(_) => {
            println!("TuneType is not running (stale PID file)");
        }
        None => {
            println!("TuneType is not running");
        }
    }
}

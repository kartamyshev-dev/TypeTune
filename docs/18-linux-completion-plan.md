...
По запросу пользователя сделан [этап 51](51-debian-package.md): готовый `.deb`,
графическая регистрация сеанса и выдача доступа, обновление/удаление с сохранением
конфигурации. Проверены isolated dpkg lifecycle, GUI и разрешение зависимостей.
Системная установка после авторизации и миграция текущего пользователя подтверждены.
Остались отдельные native проверки нового входа и package upgrade/remove.
Остальные задачи ниже отложены по запросу пользователя.

### Обновление приоритета: GUI перед пользовательскими словарями

По запросу пользователя выполнен [первый GUI срез](43-preview-gui.md): статус,
запуск/остановка, пауза и автокоррекция через общий controller; ярлык в меню.
Выполнен [срез слов/исключений](47-user-dictionaries.md): сохранение с generation
...
Статус: **частично реализован** — [checkpoint 40](40-autocorrection-lexicons.md):
частотные списки, индекс, корпус и установка; пользовательские исключения добавлены
в этапе 47. Калибровка уверенности и широкий независимый корпус ещё впереди. Добавлен 2026-09-16 после пользовательской
проверки: ручное переключение работает, но многие слова не исправляются автоматически.
На старте этапа matcher использовал лишь 150 русских и 149 английских слов, длину 4–32
...
1. **Зафиксировать baseline качества.** Создать размеченный корпус RU→EN и EN→RU:
   распространённые слова и словоформы, регистр, короткие слова, имена, термины,
   неоднозначные пары, URL/email/пути/код. Сначала добавить воспроизводящие тесты
   пропущенных замен. Отделить примеры для настройки от контрольной выборки.
2. **Подготовить полноценные словари.** Выбрать источники со словоформами и
   частотностью; до включения проверить лицензию и право распространения.
   Зафиксировать версию, происхождение, контрольные суммы и воспроизводимую
   обработку. Проверять UTF-8, дубликаты, алфавит и некорректные формы.
   Старые baseline-словари не принимать без такой проверки.
3. **Расширить общий matcher.** Загружать подготовленные данные вне пути обработки
   физических событий; поиск не должен выполнять файловый или сетевой I/O при
   вводе. Использовать частотность и оценку уверенности; пороги выбирать по корпусу.
   Короткие и неоднозначные слова включать только после отдельной проверки качества.
4. **Добавить пользовательское управление.** Слова и исключения хранить отдельно
   от пакетных данных; сохранять настройки между запусками через общий controller.
   После ручной отмены не исправлять тот же token повторно в текущем контексте.
5. **Проверить интеграцию.** Сохранить Space как текущую границу автокоррекции,
   Double Shift как независимое от словаря ручное переключение и повторный toggle.
   Проверить текст, каретку, пробел и язык дальнейшего ввода в обоих режимах.

Критерии завершения:

- Опубликовано сравнение baseline и новой версии на одной контрольной выборке:
  число нужных замен, пропусков и ошибочных замен; охват и точность с размером корпуса.
- Все обязательные негативные fixtures сохраняют исходный текст; положительные
  проверяют результат и каретку в контролируемом editor с fake clock.
- Повреждённый/отсутствующий словарь явно отключает автокоррекцию; ручная коррекция
  и обычная печать продолжают работать.
- Unit/simulated результаты и native acceptance записаны отдельно с commit,
  версиями ОС/сессии/приложения и IDs cases; пользовательский текст не логируется.
- Обновлены документация словарей, протокол проверок и инструкция установки.

Исправление абсолютно всех слов не является критерием: неоднозначные сочетания
могут требовать ручного решения. Расширение словаря не доказывает поддержку всех приложений.
...
3. Настройки/диагностика в UI, выбор режима и исключения чувствительных приложений.
4. Другие Linux desktops и standalone X11 — отдельные backend/acceptance этапы.

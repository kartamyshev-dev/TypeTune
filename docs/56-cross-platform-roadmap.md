...
Дополнение 2026-09-18: приоритет изменён на macOS для доступного Apple Silicon
стенда. [Checkpoint 57](57-macos-checkpoint.md) фиксирует реализованную локальную
сборку и частичную приёмку; native text cases остаются pending.
...
Порядок: **CP-1 → локальные macOS M-1…M-3 → Windows W-1…W-4 → macOS M-4 → CP-2**.

...
Текущий следующий срез (2026-09-16): [качество автокоррекции RU/EN](18-linux-completion-plan.md#следующий-этап-качество-автокоррекции-ruen) — словари, корпус, оценка уверенности и пользовательские исключения. Статус: первый срез выполнен — [частотные словари и корпус](40-autocorrection-lexicons.md); оставшиеся задачи указаны в рабочем плане.

Выполнен [срез 49 — исключения приложений в GUI](49-application-exclusions.md):
72 Python tests, GTK picker и live controller ACK; native замены с исключениями
в сторонних приложениях ожидают отдельной проверки.

Реализован [срез 50 — предложения после трёх ручных возвратов](50-correction-feedback.md).
Постоянное исключение требует отдельного подтверждения в GUI.

По запросу пользователя приоритет перенесён на [пакет Debian и GUI setup](51-debian-package.md).
Изолированный package lifecycle проверен; native системная установка/вход остаются отдельной приемкой.

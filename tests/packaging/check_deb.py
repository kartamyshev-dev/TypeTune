...
with tempfile.TemporaryDirectory(prefix='typetune-dpkg-') as temporary:
    ...
    for name in ('settings.json','words.json','applications.json','feedback.json'):(user/name).write_text('fixture-preserved\n')
    ...
    # via build.rs. Ship no /usr/share/typetune/dict/ tree (negative assert).
    assert not (root/'usr/share/typetune/dict').exists(), 'runtime dict/ must stay absent'
    # CC BY-SA / CC BY attribution for embedded frequency data (plan 57/P3).
    freq_docs = root/'usr/share/doc/typetune-preview/frequencywords'
    assert freq_docs.is_dir(), freq_docs
    freq_names = {p.name for p in freq_docs.iterdir() if p.is_file()}
    assert any(n.lower().startswith('license') for n in freq_names), freq_names
    assert 'manifest.json' in freq_names, freq_names
    assert 'README.md' in freq_names, freq_names
    assert 'LICENSE-leeds.txt' in freq_names, freq_names

...
> Общесистемная коррекция раскладки русский ↔ английский.
...
TypeTune исправляет текст, набранный не в той раскладке (например, `ghbdtn` → `привет`) и переключает источник ввода.
...
- **Строка меню / трей** — пауза, флаг раскладки (`🇺🇸` / `🇷🇺` / `?`), необязательный звук переключения
- **Нативные настройки** (Linux) — `typetune-gui` и `typetune doctor --session`

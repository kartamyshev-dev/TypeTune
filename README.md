...
Версия **preview57-2** включает macOS-приложение со строкой меню, предложения для словаря,
пробел до/после Double Shift, удаление IBus-режима и один ярлык в меню приложений.
...
файлами. Откройте `.deb` графическим установщиком, затем **TypeTune**
в меню приложений. При необходимости настройка откроется автоматически; повторно
её можно открыть через контекстное меню ярлыка → «Настроить доступ и сеанс». Rust и сборка исходников пользователю не нужны.
